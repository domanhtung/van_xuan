...
const BlogNews = () => {
  return (
    <div className="mt-40 py-20 bg-[#F9F9F9]">
      <div className="container mx-auto">
        <p className="text-[#FF375F] text-center font-semibold">Blog & News</p>
          ...
          {blogsPreview?.map((item, index: number) => {
            const time = useMemo(() => {
              return {
                ...
                month: item?.time?.toLocaleString("en-US", { month: "short" }),
              };
            }, [item?.time]);
              ...
              >
                <div className="relative w-full h-[300px] overflow-hidden">
                  <Image
                    ...
                    alt="blog"
                  />
                  <div className="absolute top-5 right-5 px-4 py-2 text-[16px] bg-[#002856] text-white rounded-[50px]">
                    {item?.subject}
                  </div>
                </div>
                ...
                <div className="px-5 pb-5 text-[24px] group-hover:text-[#FF375F] text-[#002856] font-bold duration-500">
                  {item?.title}
                </div>
              </div>
            );
